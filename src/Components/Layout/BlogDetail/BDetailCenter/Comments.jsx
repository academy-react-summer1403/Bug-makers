...
import { commentDissLikeNews, commentLikeNews, coomentDelLikeNews, getRepleyComment } from '../../../../Core/Services/Api/BlogDetail/CommentDetail';
...
const Comments = ({
            ...
            pictureAddress,newsId,userId,parentId,currentUserLikeId,GetComment,renderCourses,
            onSubmit}) => {
            ...
            const delLikeNews2 = async()=>{
                ...
                const res = await coomentDelLikeNews({"deleteEntityId":`${currentUserLikeId}`});       
                console.log(res)
                GetComment()
            ...
            const showRepley =async(id)=>{
                ...
                {/* Header: نام نویسنده، تاریخ، ساعت */}
                    ...
                    <div className="text-[0.7vw] text-[#5E5E5E] flex justify-between w-[50%]">
                        <span className="max-w-[5vw]">{title}</span>
                        |<span>۲۵ اردیبهشت ۱۴۰۳ </span>
                        |<span>ساعت ۱۶:۲۴  </span>
                        <span className="mr-[0.5vw]">۲ روز پیش</span>
                    ...
                    <div className="text-[0.8vw] gap-[1vw] text-gray-800 w-1/3 h-[1.46vw]  flex justify-end">
                        <div className="flex justify-evenly h-full w-[25%] items-center">
                            <span>{likeCount}</span>
                            <svg onClick={()=>{currentUserIsLike ? delLikeNews2() :setNewsLike()} }  className="cursor-pointer"  width="1.51vw" height="1.3vw" viewBox="0 0 29 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                fill={currentUserIsLike ? "#FF0000" : "#7F7F7F"}/>
                            </svg>
                        ...
                        <div className="flex justify-evenly h-full w-[25%] items-center">
                            ...
                            <svg onClick={()=>{currentUserIsDissLike ? delLikeNews2() :setNewsDissLike()} } className="cursor-pointer" width="1.51vw" height="1.3vw" viewBox="0 0 29 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                fill={currentUserIsDissLike ? "#FF0000" : "#7F7F7F"}/>
                            </svg>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex justify-between items-center">
                    ...
                    <button onClick={()=>{showRepley(id)}} className="text-[0.7vw] bg-[#F6F6F6] text-gray-500 hover:text-gray-900 rounded-[0.78vw] w-[7vw] h-[1.7vw] p-0">
            ...
            <div className={repleyModal?'fixed z-0 top-0 left-0 h-screen w-screen bg-[#8a8a8a96] backdrop-blur-[3px]':'hidden'} onClick={()=>{setRepleyModal(false)}}>
                <div className={repleyModal?'h-max w-[50vw] rounded-[1vw] bg-white absolute z-40 top-1/4 left-1/4':'hidden' }onClick={(e) => e.stopPropagation()}>
                    <AddCommentForm  userId={userId} onSubmit={onSubmit} newsId={newsId} parentId={parentId}/>
                </div>
            </div>
            <div>
                {
